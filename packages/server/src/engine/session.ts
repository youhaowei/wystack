 ...
 * Mirrors the shipped WebSocket behavior in `routes.ts` for parity over Pipe.
 */
...
/**
 * Loose envelope parse — matches `routes.ts` pre-dispatch policy (non-object,
 * array, primitive, or non-string `type` → null).
 */
...
export function createSession(opts: SessionOptions): Session {
  ...
  async function resolveSubContext(connToken: string | null): Promise<Record<string, unknown>> {
  ...
  async function handleAuthFrame(msg: Record<string, unknown>): Promise<void> {
    ...
    try {
      const resolved = await resolveSubContext(parsedToken)
      if (closed) return
      if (authenticated) {
        send({ type: 'authenticated' })
        return
