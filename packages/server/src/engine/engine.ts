/**
 * Engine — wires Session + Dispatch to a `Pipe` for the RPC tier.
 * Reactive tier is not wired here yet; subscribe returns REACTIVITY_NOT_ENABLED.
 */
...
export interface EngineOptions {
  app: WyStackApp
  /**
   * Synthetic upgrade request for `resolveContext`. Loopback callers can omit
   * it; WebSocket adapters pass the real HTTP upgrade request.
   */
...
export function attachEngine(pipe: Pipe, opts: EngineOptions): () => void {
  ...
  const upgradeRequest = opts.upgradeRequest ?? new Request('http://loopback/ws', { method: 'GET' })
