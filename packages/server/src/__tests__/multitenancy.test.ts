...
beforeEach(async () => {
  ...
  app = await wy.build({
    ...
    functions: {
      addInsight: wy.procedure.input({ id: uuid, name: text }).mutation(async (ctx, args) =>
        ctx.db.into(schema.insights).insert(args),
      ),
      listInsights: wy.procedure.input({}).query(async (ctx) => ctx.db.from(schema.insights).all()),
      addCatalog: wy.procedure.input({ id: uuid, name: text }).mutation(async (ctx, args) =>
        ctx.db.into(schema.catalog).insert(args),
      ),
    },
  })
})
