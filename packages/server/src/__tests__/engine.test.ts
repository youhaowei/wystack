...
function nextMessage<T>(messages: T[], predicate: (m: T) => boolean, ms = 5000): Promise<T> {
  return new Promise((resolve, reject) => {
    ...
    const interval = setInterval(() => {
      check()
      if (messages.find(predicate)) clearInterval(interval)
    }, 10)
    check()
  })
}
...
describe('attachEngine over loopback', () => {
  ...
  test('RPC call on no-auth pipe returns result', async () => {
    const [client, server] = createLoopbackPair<ClientMessage, ServerMessage>()
  ...
  test('idempotent auth ACK on no-auth server does not adopt token into call context', async () => {
    const [client, server] = createLoopbackPair<ClientMessage, ServerMessage>()
    const { messages, unsub } = collectMessages(client)
    attachEngine(server, { app })
  ...
  test('malformed first frame closes pipe when auth required', async () => {
    const [client, server] = createLoopbackPair<ClientMessage, ServerMessage>()
    let serverClosed = false
    const origClose = server.close.bind(server)
    server.close = () => {
