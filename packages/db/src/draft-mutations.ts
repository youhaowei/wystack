...
async function materializeDraftInsertDefaults(
  db: DrizzleDb,
  ...
  for (const [property, column] of Object.entries(columns)) {
    ...
    if (typeof column.defaultFn === 'function') {
      row[property] = await column.defaultFn()
      continue
    }
    ...
    if (
      ...
      const result = await db.execute(sql`SELECT ${column.default} AS "value"`)
      const value = normalizeExecuteRows(result)[0]?.['value']
      row[property] = mapColumnValueFromDriver(column, value)
      continue
    }
...
export class DraftInsertBuilder<T extends AnyTable> {
  ...
  async insert(
    ...
    for (const row of rows) {
      ...
      const r = await materializeDraftInsertDefaults(
        this._db,
        columns,
        withRevision,
        pkPropKey ?? pkColName,
